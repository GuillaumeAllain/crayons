...
class TestPropagationSph(unittest.TestCase):
    def test_find_intersection(self):
            ...
            np.all(
                np.isclose(
                    propagation.find_intersection(
                        ray,
                        lambda x, y: sph["sag"](x, y, c=-0.25),
                        lambda x, y: sph["normal"](x, y, c=-0.25),
                        t=1.5,
                    ),
                    [0, 0, 0],
                )
        ...
        ray = Ray(*(0, 0, 0, 0.12635241, 0.41923115, 0.89904411))
        self.assertTrue(
            np.all(
                np.isclose(
                    propagation.find_intersection(
    ...
    def test_transfert(self):
        ...
        ray = Ray(*(-0.21081125, -0.69946149, 0.00000000, 0.12635241, 0.41923115, 0.89904411))
        self.assertTrue(
            np.all(
                np.isclose(
                    propagation.transfert(
                        ray,
                        lambda x, y: sph["sag"](x, y, c=0.5),
                        lambda x, y: sph["normal"](x, y, c=0.5),
                        t=3,
                    ).vector,
                    [
                        0.23521425,
